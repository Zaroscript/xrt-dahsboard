...
import type { User, UserStatus } from '@/store/slices/usersSlice';
...
export default function Users() {
  ...
  const exportUsers: ExportUser[] = filteredAndSortedUsers.map(user => ({
    ...
    lastActive: user.lastActive ? new Date(user.lastActive).toLocaleString() : 'N/A',
  }));
  const exportHeaders = [
    'ID', 'Name', 'Email', 'Status', 'Role', 'Created At', 'Last Active'
  ] as const;
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <CSVLink
              ...
              headers={exportHeaders.map(header => ({ label: header, key: header }))}
            >
        ...
        >
          <TabsList className="grid w-full grid-cols-5 h-auto p-1 bg-muted/20">
            ...
            <TabsTrigger value="rejected" className="flex items-center gap-2">
              ...
              Rejected
              <Badge variant="secondary">
                {users.filter(u => u.status === 'rejected' && !u.subscription).length}
              </Badge>
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="pending" className="mt-6">
            <UserList
              users={filteredAndSortedUsers.filter(u => u.status === 'pending')}
              onUpdate={handleUserUpdateWithReason}
          ...
          <TabsContent value="active" className="mt-6">
            <UserList
              users={filteredAndSortedUsers.filter(u => u.status === 'active')}
              onUpdate={handleUserUpdateWithReason}
          ...
          <TabsContent value="blocked" className="mt-6">
            <UserList
              users={filteredAndSortedUsers.filter(u => u.status === 'blocked')}
              onUpdate={handleUserUpdateWithReason}
          ...
          <TabsContent value="rejected" className="mt-6">
            <UserList
              users={filteredAndSortedUsers.filter(u => u.status === 'rejected')}
              onUpdate={handleUserUpdateWithReason}
